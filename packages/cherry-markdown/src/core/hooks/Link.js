...
export default class Link extends SyntaxBase {
  ...
  toHtml(match, leadingChar, text, link, title, target) {
    ...
    let attrs = title && title.trim() !== '' ? ` title="${_e(title.replace(/["']/g, ''))}"` : '';
    if (target) {
    ...
    if (isValidScheme(processedURL)) {
      ...
      // urlProcessor is an extension point and may rewrite an otherwise safe URL.
      // Validate the post-processed value before it reaches href.
      if (!isValidScheme(processedURL)) {
