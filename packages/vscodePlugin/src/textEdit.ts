...
export function calculateTextReplacement(currentText: string, nextText: string): TextReplacement | undefined {
  ...
  let startOffset = 0;
  while (startOffset < sharedLimit && currentText[startOffset] === nextText[startOffset]) startOffset += 1;
  ...
  return { startOffset, endOffset: currentEnd, text: nextText.slice(startOffset, nextEnd) };
}
